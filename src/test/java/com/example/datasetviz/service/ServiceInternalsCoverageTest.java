...
class ServiceInternalsCoverageTest {
    ...
    void csvServicePrivateMethodsCoverRemainingBranches() throws Exception {
        ...
        CSVRecord unmappedRecord = mock(CSVRecord.class);
        when(unmappedRecord.isMapped("Value")).thenReturn(false);
        assertThat(invoke(service, "isNumericColumn", new Class[]{String.class, List.class}, "Value", List.of(unmappedRecord))).isEqualTo(false);

        CSVRecord blankRecord = mock(CSVRecord.class);
        when(blankRecord.isMapped("Value")).thenReturn(true);
        when(blankRecord.get("Value")).thenReturn("   ");
        assertThat(invoke(service, "isNumericColumn", new Class[]{String.class, List.class}, "Value", List.of(blankRecord))).isEqualTo(false);

        CSVRecord numericRecord = mock(CSVRecord.class);
        when(numericRecord.isMapped("Value")).thenReturn(true);
        when(numericRecord.get("Value")).thenReturn("7");
        ...
        when(blankRecord.isMapped("Location")).thenReturn(true);
        when(blankRecord.get("Location")).thenReturn("   ");
        assertThat(invokeStatic(CsvAnalyticsService.class, "resolveLocation", new Class[]{CSVRecord.class, String.class}, blankRecord, "Location")).isNull();

        Class<?> fileSchemaClass = Class.forName("com.example.datasetviz.service.CsvAnalyticsService$FileSchema");
        ...
        Class<?> mutableAnalyticsClass = Class.forName("com.example.datasetviz.service.CsvAnalyticsService$MutableAnalytics");
    ...
    void emailAnalyticsMutableAnalyticsCoversNormalizationAndDateComparisons() throws Exception {
        Class<?> mutableAnalyticsClass = Class.forName("com.example.datasetviz.service.EmailAnalyticsService$MutableAnalytics");
