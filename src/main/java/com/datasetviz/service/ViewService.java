...
public class ViewService {
    ...
    public DatasetView toDatasetView(DatasetRegistration registration, Boolean hdfsPathAlreadyExisted) {
        return new DatasetView(
                ...
                !registration.getPendingLocalImports().isEmpty(),
                registration.getPendingLocalImports().stream()
                        .map(importEntry -> new DatasetView.PendingLocalImportView(importEntry.getLocalPath(), importEntry.getTargetSubdirectory()))
                        .toList()
        );
    }
